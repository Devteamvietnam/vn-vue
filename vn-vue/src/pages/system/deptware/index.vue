...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDeptWare(this.queryParams).then((response) => {
    ...
    handleUpdate(row) {
      this.reset();
      getDeptWare(row.deptId).then((response) => {
        this.form = response.data;
        this.open = true;
        this.title = "Modify Department";
      });
      listDeptWareExcludeChild(row.deptId).then((response) => {
        this.deptOptions = this.handleTree(response.data, "deptId");
      });
    },
    /** Submit button */
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.deptId != undefined) {
            updateDeptWare(this.form).then((response) => {
              this.msgSuccess("Modified successfully");
              this.open = false;
              this.getList();
            });
          } else {
            addDeptWare(this.form).then((response) => {
